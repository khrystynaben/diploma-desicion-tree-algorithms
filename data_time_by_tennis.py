...
train_data_tennis = pd.read_csv("D:/university/diploma/data/tennis/new_train.csv") 
test_data_tennis = pd.read_csv("D:/university/diploma/data/tennis/new_test.csv") 
...
# ----------------2--C45-----------------------
X_emp = train_data_tennis.drop(columns=['Play Tennis']).values
y_emp = train_data_tennis['Play Tennis'].values
X_emp_test = test_data_tennis.drop(columns=['Play Tennis']).values
...
#print(treeC45)
accuracyC45 = new_evaluateС45(treeC45, test_data_tennis, 'Play Tennis') 
...
treeCart = new_cart(train_data_tennis.values.tolist(), 100, 1)
#print(treeCart)
end_time3 = time.time()  # Кінець вимірювання часу
execution_time3 = end_time3 - start_time3
print(f"Час виконання алгоритму CART: {execution_time3} секунд")
predictions = []
for index, row in test_data_tennis.iterrows():
    prediction = predict(treeCart, row)
    predictions.append(prediction)

# Приклад використання
actual_values = test_data_tennis['Play Tennis'].values.tolist()  # Справжні значення міток
...
predictions = chaid.predict(test_data_tennis)
actual = test_data_tennis['Play Tennis'].values.tolist()
...
# -----------------6-Forest-----------------------
predictions = []
data = train_data_tennis.values.tolist()
test_data = test_data_tennis.values.tolist()
