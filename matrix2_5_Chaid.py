...
def make_predictions0(model, t_data):
    predictions = []
...
def make_predictions(model, X):
    ...
    for index, row in X.iterrows():
        prediction = model.root.predict(row)
        predictions.append(prediction)
    return predictions

# Фактичні та передбачені значення
actual_values = test_data[label].values.tolist()
predicted_values = make_predictions(model, test_data)
...
print("Accuracy: ", accuracy_chaid(actual_values, make_predictions(model, test_data)))
